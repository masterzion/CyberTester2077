...
const inputInteractions = require('./input-interactions.cjs');
const { TESTER_SYSTEM_PROMPT } = require('./tester-system-prompt.cjs');
...
const RUN_DIR = outputDirectory(OUTPUT_ROOT, RUN_ID, process.env.AUDIT_ACCOUNT_ROLE || 'child', process.env.AUDIT_ACCOUNT_NAME, process.env.CHILD_EMAIL, process.env.AUDIT_PLATFORM || 'web', process.env.AUDIT_SCREEN_PROFILE);
...
async function waitForPageReady(page) {
  ...
  // Background polling must not delay a loaded page. Individual actions still
  // wait for their target to become visible, enabled, and actionable.
  await page.locator('body').waitFor({state:'visible',timeout:Math.max(1,deadline-Date.now())});
}
...
function executionCandidates(controls, attempted) {
  return controls.filter(control => (control.visible || control.type === 'file') && !control.disabled && !inputInteractions.exclusion(control) && !attempted.has(controlKey(control)));
}
async function inspect(page, origin) {
  ...
  while(actions < cfg.maxActions) {
    ...
    const emptyTarget=current.find(c=>c.visible && !inputInteractions.exclusion(c) && inputInteractions.isSubmit(c) && !emptyProbed.has(controlKey(c)));
    if(emptyTarget) {
      emptyProbed.add(controlKey(emptyTarget));
      const result=await inputInteractions.probeEmptySubmission(page,emptyTarget,cfg.actionTimeoutMs).catch(error=>({status:'FAIL',detail:'Empty-input check: '+error.message,scenario:'empty'}));
      if(result) {
        actions++;
    ...
    candidates.sort((a,b)=>inputInteractions.priority(a)-inputInteractions.priority(b) || Number(guided.has(controlKey(b)))-Number(guided.has(controlKey(a))));
    const control=candidates[0];
    if(!control) {
      if(scrollPasses++ >= 3) break;
...
module.exports = { cfg, settings, DOCS_ROOT, matchCurrentControl, executionCandidates, controlKey, waitForPageReady };
